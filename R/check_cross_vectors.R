...
#' @export
check_length_vecs <- function(vec1, vec2, vec1_arg = "vec1", vec2_arg = "vec2", raise = "error", alert_message = NULL){
...
#' @return invisible NULL
#' @export
check_equality_vecs <- function(vec1, vec2, vec1_arg = "vec1", vec2_arg = "vec2", raise = "error", alert_message = NULL){
  if(any(vec1 != vec2)){
    if(is.null(alert_message)){
