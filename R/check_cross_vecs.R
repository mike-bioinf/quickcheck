...
#' Note that unique arg has precedence over the recycle one, meaning the duplicate values are removed before the check on lengths is performed.
...
check_equality_vecs <- function(vec1, vec2, unique = FALSE, sort = FALSE, recycle = FALSE, coerce = FALSE, vec1_arg = "vec1", vec2_arg = "vec2",
  ...
  if(any(is.na(vec1)) || any(is.na(vec1))){
...
#' @return invisible NULL
#' @export
check_unordered_equality_vecs <- function(vec1, vec2, na_rm = FALSE, coerce = FALSE, vec1_arg = "vec1", vec2_arg = "vec2", 
                                          raise = "error", alert_message = NULL, quickalert = TRUE, n_evaluation_frame = 0, ...){
  check_required_all()
